...
- 6814328: Keep a dragged object tracking the pointer on documents where moving it changes much of the page.
    ...
    Positions on a visible graph are computed on every step, not estimated, so constraints and snapping still apply as you drag. What waits is the redrawing of what is off the visible graphs, and the recomputation that redrawing drives. The deferred work always completes on release, so nothing is left stale.
...
- d346171: Bundle Comlink into the iframe's boot script instead of importing it from unpkg at runtime.
...
- 434b175: Iframe: make embedded viewer/editor body backgrounds follow light, dark, and system theme settings so partially filled iframes no longer show white footer stripes.
- 8f51194: Windowed viewers (`mountPolicy`) now mount lazily with a boot-concurrency
  ...
  boot transient.
- ace1982: `<DoenetViewer>` prop changes no longer reload the iframe. The wrapper now
...
- 3a13674: Windowed mounting (`mountPolicy`) improvements for embedding hosts:
...
- c58aedc: New opt-in `mountPolicy` prop on `<DoenetViewer>`: windowed mounting. With
...
- b9082b7: Minify the published `@doenet/standalone` bundle.
...
- 9ba9539: New `coordinator.js`, published alongside the standalone bundle: a small
